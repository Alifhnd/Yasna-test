...
abstract class Module extends ServiceProvider
{
    ...
    {
    // @TODO we try to find active modules from database
    if ($this->getName() == 'Yasna') {
        return true;
    }
    try {
        $setting = new \Modules\Yasna\Entities\Setting();
        $setting = $setting->ask('active_modules');
        //$setting = \Modules\Yasna\Entities\Setting::where('slug', 'active_modules')->first();
    } catch (\PDOException $exception) {
        $setting = false;
    }

    if ($setting and $setting->exists) {
        $module_array = explode('|', $setting->custom_value);
        return in_array($this->getName(), $module_array);
    } else {
