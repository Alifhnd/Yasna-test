...
trait PermitsTrait
{
    ...
    */
    ...
    {
        $default_role = role(self::defaultRole());
    ...
    {
        return strtr($text, config('yasna.permit_alpha'), config('yasna.permit_coder'));
    }
    ...
    {
        return strtr($text, config('yasna.permit_coder'), config('yasna.permit_alpha'));
    }
    ...
    */
    ...
    public function setPermission($permissions)
    {
        ...
        $this->roles()->updateExistingPivot($current_row['id'], [
             ...
             'key'         => $this->makeKey($current_row['id'], $permissions, $current_row['pivot']['status'],
                  $current_row['pivot']['deleted_at']),
        ])
        ;
    ...
    public function setStatus($status)
    {
        ...
        if (!$as) {
            ...
            if ($as == self::defaultRole()) {
                $this->update([
                     'default_role_key'    => $this->makeKey($current_row['id'], '', $status,
                          $current_row['pivot']['deleted_at']),
            ...
            $this->roles()->updateExistingPivot($current_row['id'], [
                 'status' => $status,
                 'key'    => $this->makeKey($current_row['id'], $current_row['pivot']['permissions'], $status,
                      $current_row['pivot']['deleted_at']),
            ])
            ;
    ...
    public function disableRole($role_slug)
    {
        ...
        if ($role_slug == self::defaultRole()) {
            $this->update([
                 'default_role_key'        => $this->makeKey($current_row['id'], '', $current_row['pivot']['status'],
                      $now),
                 'default_role_deleted_at' => $now,
            ]);
        } else {
            $this->roles()->updateExistingPivot($current_row['id'], [
                 'key'        => $this->makeKey($current_row['id'], $current_row['pivot']['permissions'],
                      $current_row['pivot']['status'], $now),
                 'deleted_at' => $now,
            ])
            ;
        }
    ...
    */
    ...
    /**
     * @param string $required_permits : any special permission that should be checked.
     ...
     */
    public function domainsQuery($required_permits = null, $min_status = 8)
    {
        /*-----------------------------------------------
        | Bypass if the user in question is a manager ...
        */
        if ($this->is_a('manager')) {
            return model('domain')->where('id', '>', '0');
        }
        ...
        */
        ...
        foreach ($roles as $role) {
            if ($required_permits) {
                if ($this->as($role)->cannot($required_permits)) {
                    continue;
                }
            }
            if (str_contains($role, model('role')->domain_role_prefix . '-')) {
                $domains_array[] = $role;
                $domains_array[] = str_replace(model('role')->domain_role_prefix . '-', null, $role);
            }
        }

        $domains = model('domain')->whereIn('slug', $domains_array);
     ...
     */
    public function domainsArray($required_permits = null, $min_status = 8)
    {
        return $this->domainsQuery($required_permits, $min_status)->get()->pluck('slug')->toArray();
    }



    /**
     * @param null $required_permits
     * @param int  $min_status
     *
     * @return integer: the number of all the domains, the user has access to
     */
    public function domainsCount($required_permits = null, $min_status = 8)
    {
     ...
     * the role is passed via the as() chain method. first one is returned, therefore not intended to support multiple
     * roles.
     */
    public function row()
    {
    ...
    public function hasRole($request = null, $any_of_them = false)
    {
        ...
        */
        ...
        if ($any_of_them) {
            ...
            return boolval($count == count((array)$request));
        }
    }
    ...
    public function can($request = '*', $blah = false)
    {
        ...
        */
        ...
        $request = str_replace('settings', 'setting', $request);
        $request = str_replace('setting', 'settings',
             $request);  // <~~ first changed 'settings' to 'setting' to avoid double s at the end.
        ...
        */
        ...
        if ($request != 'dev' and $request != 'developer' and $this->isSuperadmin()) {
            return true;
        }
        ...
        */
        ...
        return str_contains($this->as($as)->rolesPermits(), $request);
    }
    ...
    */
     ...
     *                        'all' can be passed to set the as_all flag, used in $this->can() to prevent accidental
     *                        call of all roles.
    ...
    */
     ...
     * @return bool
     */
    public function is($requested_role)
    {
    ...
    {
        return $this->is_any_of(model('role')::adminRoles());
    }
    ...
    {
        $as      = $this->getChain('as');
        $role_id = $this->withDisabled()->as($as)->rolesQuery()->first()['pivot']['role_id'];
        $role    = model('role', $role_id);

        if (!$role->exists) {
            return 'unknown';
        } elseif ($this->as($as)->disabled()) {
            return 'banned';
        } else {
            return $role->statusText($this->as($as)->getStatus());
        }
    }
