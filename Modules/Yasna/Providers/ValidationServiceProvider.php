...
{
    private static $default_global_rules = 'trim|stripArabic';
    private $input;
    private $rules;
    ...
    private function applyFilter($key, $rule)
    {
        ...
        switch ($rule) {
            ...
            case 'date':
                if ($data) {
                    try {
                        $data = Carbon::createFromTimestamp(floor($data / 1000))->toDateTimeString();
                    } catch (Exception $e) {
             ...
             */
            case 'gDate':
                if ($data and strlen($data) >= 6 and ($jDate = explode('/', $data)) and (count($jDate) == 3)) {
            ...
            case 'dehash':
                if ($data) {
                    ...
                    if ($dehashed and is_array($dehashed) and count($dehashed) and is_numeric($dehashed[0])) {
                        $data = $dehashed[0];
                    } else {
                        $data = null;
                    }
